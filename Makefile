...
PLT_NAME=.{{appid}}_dialyzer.plt

$(PLT_NAME):
	@ERL_LIBS=deps dialyzer --build_plt --output_plt .{{appid}}_dialyzer.plt \
		--apps kernel stdlib sasl lager || true

dialyze: $(PLT_NAME)
	@dialyzer apps/{{appid}}/ebin --plt $(PLT_NAME) --no_native \
		-Werror_handling -Wrace_conditions -Wunderspecs

#  http://erlang.org/pipermail/erlang-questions/2010-October/053853.html
d1: $(PLT_NAME)
	@dialyzer apps/{{appid}}/ebin --plt $(PLT_NAME) --no_native --no_spec \
		-Werror_handling -Wrace_conditions -Wunderspecs
